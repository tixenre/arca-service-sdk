...
async def test_set_facturacion_campo_nunca_configurado_viene_null(client, httpx_mock):
    ...
    result = await client.set_facturacion("cliente-1", iibb="901-123456-7")
    assert result.iibb == "901-123456-7"
    assert result.nombre_comercial is None


async def test_set_facturacion_iibb_nunca_configurado_viene_null(client, httpx_mock):
    httpx_mock.add_response(
        method="PUT",
        url=f"{_API}/clientes/cliente-1/facturacion",
...
async def test_generar_csr_409_csr_ya_existe(client, httpx_mock):
    httpx_mock.add_response(
        method="POST",
        url=f"{_API}/clientes/cliente-1/csr",
        match_json={"cuit": "20301234563", "regenerar": False},
    ...
    with pytest.raises(CsrYaExisteError) as exc_info:
        await client.generar_csr("cliente-1", "20301234563")
    assert exc_info.value.status_code == 409
    assert isinstance(exc_info.value, RequestError)


async def test_generar_csr_409_credencial_ya_activa(client, httpx_mock):
    httpx_mock.add_response(
        method="POST",
        url=f"{_API}/clientes/cliente-1/csr",
        match_json={"cuit": "20301234563", "regenerar": False},
...
async def test_generar_csr_regenerar_evita_el_409(client, httpx_mock):
    httpx_mock.add_response(
        method="POST",
        url=f"{_API}/clientes/cliente-1/csr",
    ...
    result = await client.generar_csr("cliente-1", "20301234563", regenerar=True)
    assert result.alias == "cliente-1-2026"
...
async def test_emitir_nota_credito_por_total_manda_un_solo_campo(client, httpx_mock):
    httpx_mock.add_response(
        method="POST",
        url=f"{_API}/clientes/cliente-1/notas-credito",
    ...
    result = await client.emitir_nota_credito_por_total("cliente-1", "factura-1")
    assert result.comprobante.tipo == "NOTA_CREDITO"


async def test_emitir_nota_credito_por_total_con_idempotency_key_explicita(client, httpx_mock):
    httpx_mock.add_response(
        method="POST",
        url=f"{_API}/clientes/cliente-1/notas-credito",
    ...
    result = await client.emitir_nota_credito_por_total(
...
async def test_listar_comprobantes_asociado_id_trae_las_notas_de_esa_factura(client, httpx_mock):
    httpx_mock.add_response(
        method="GET",
    ...
    result = await client.listar_comprobantes("cliente-1", asociado_id="factura-1")
...
async def test_crear_sesion_embebida_comprobante_con_receptor(client, httpx_mock):
    ...
    await client.crear_sesion_embebida_comprobante("cliente-1", sesion)
...
async def test_401_levanta_credentials_rejected_error(client, httpx_mock):
    httpx_mock.add_response(
        method="POST",
        url=f"{_API}/clientes/cliente-1/comprobantes",
        status_code=401,
...
async def test_403_origen_no_verificado_levanta_credentials_rejected_error(client, httpx_mock):
    httpx_mock.add_response(
        method="GET",
        url=f"{_API}/clientes/cliente-1/comprobantes/factura-1",
        status_code=403,
